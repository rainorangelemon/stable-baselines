...
def deconv(input_tensor, scope, *, n_filters, filter_size, stride,
         pad='VALID', output_height, output_width, init_scale=1.0, data_format='NHWC', one_dim_bias=False):
    """
    Creates a 2d convolutional layer for TensorFlow
    ...
    with tf.variable_scope(scope):
        ...
        deconv_shape = tf.stack([batch_size, output_height, output_width, n_filters])
        return bias + tf.nn.conv2d_transpose(input_tensor, weight, output_shape=deconv_shape, strides=strides, padding="VALID", data_format=data_format)
...
def linear_general(input_tensor, scope, n_hidden, *, init_scale=1.0, init_bias=0.0):
    """
    Creates a fully connected layer for TensorFlow
